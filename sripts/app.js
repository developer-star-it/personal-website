let navToggleIcon = document.querySelector(".nav__toggle-icon");
...
function navigationTabsInit(listItems, listItemActiveClass, listItemShowClass) {
  listItems.forEach((listItem) => {
    listItem.addEventListener("click", function () {
      ...
      document
        .querySelector(`.${listItemShowClass}`)
        .classList.remove(listItemShowClass);
...
function removeClassActive(classNameActive) {
  document
...
menuItems.forEach((menuItem) => {
  menuItem.addEventListener("click", function (e) {
    e.preventDefault();
    removeClassActive("menu__item--active");
    this.classList.add("menu__item--active");
    let sectionClass = menuItem.getAttribute("data-section");
    let sectionOffsetTo = document.querySelector(`.${sectionClass}`).offsetTop;

    if (sectionClass == "home") {
      window.scrollTo({
        top: sectionOffsetTo - 200,
      });
    } else {
      window.scrollTo({
        top: sectionOffsetTo - 120,
      });
    }
  });
});
...
changeThemeBtn.addEventListener("click", function () {
  ...
  if (document.documentElement.classList.contains("dark-theme")) {
    ...
    changeThemeBtn.innerHTML = lightThemeBtn;
    changeThemeBtn.classList.add("change-theme--header--dark")
  } else {
    window.localStorage.setItem("theme", "light-theme");
    changeThemeBtn.innerHTML = darkThemeBtn;
     changeThemeBtn.classList.remove("change-theme--header--dark")
  }
});
